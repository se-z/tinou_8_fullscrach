...
public class Space implements Cloneable{
    ...
    private class Position {//座標の情報を持つクラス
        ...
        public boolean Equal(Position aComp){
            if(aComp.getX() == mX && aComp.getY() == mY){
                return(true);
            }
            else{
                return(false);
    ...
    public void addBlock(String aID, int aX, int aY) throws IllegalArgumentException {
        ...
        if (aX < mSpaceSize.get("x")[0] || aX > mSpaceSize.get("x")[1]
                || aY < mSpaceSize.get("y")[0] 
                || aY > mSpaceSize.get("y")[1]) {
    ...
    //ブロックの配置が同じかどうか比べる １つでも違うなら真、全て同じなら偽を返す
    public boolean compareTo(Space aComp){
        //return(aComp.mBlocks.equals(mBlocks));
        //*
        boolean diff = false;//違いがあるかどうか
        for(HashMap.Entry<String, Position> tCompBlock:aComp.mBlocks.entrySet()){
            if(!mBlocks.get(tCompBlock.getKey()).Equal(tCompBlock.getValue()) ){//同じブロックで配置が異なった場合
                diff = true;
            }
        }
        return(diff);
